   ...
   <small><br>
	...
	else
	{
        ...
        if ($_REQUEST["show_canceled"] === 'Y' && count($arResult['ORDERS'])) {
            ?>
            <h3 class="page-header page-header-top"><?= Loc::getMessage('SPOL_TPL_ORDERS_CANCELED_HEADER') ?></h3>
            <h1 class="sale-order-title">
            ...
            <div class="col-md-12">
                <div id="example-datatables3_wrapper" class="">
                    ...
                    <table id="order-cancel"
                        ...
                        <thead>
                        <tr role="row">
                            <th class="text-center sorting_asc" tabindex="0" aria-controls="example-datatables3"
                                rowspan="1"
                                colspan="1" aria-sort="ascending" aria-label="#: activate to sort column descending"
                                style="width: 90px;">О Заказе
                            <th class="sorting" tabindex="0" aria-controls="example-datatables3" rowspan="1" colspan="1"
                                aria-label=" Username: activate to sort column ascending" style="width: 296px;">Цена
                            </th>
                            <th class="sorting" tabindex="0" aria-controls="example-datatables3" rowspan="1" colspan="1"
                                aria-label=" Username: activate to sort column ascending" style="width: 296px;">Статус
                            </th>
                            <th class="sorting" tabindex="0" aria-controls="example-datatables3" rowspan="1" colspan="1"
                                aria-label=" Username: activate to sort column ascending" style="width: 296px;">Действия
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        foreach ($arResult['ORDERS'] as $key => $order) {
                            if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $_REQUEST["show_canceled"] !== 'Y') {
                                ...
                                ?><!-- &laquo;--><?//=htmlspecialcharsbx($arResult['INFO']['STATUS'][$orderHeaderStatus]['NAME'])
                                ?><!--&raquo;-->
                                <!--				</h1>-->
                                <?
                            }
                            ...
                            <tr role="row" class="odd">
                                <td><?= Loc::getMessage('SPOL_TPL_ORDER')?>
                           <?= Loc::getMessage('SPOL_TPL_NUMBER_SIGN')?>
  <?= htmlspecialcharsbx($order['ORDER']['ACCOUNT_NUMBER'])?> <?= Loc::getMessage('SPOL_TPL_FROM_DATE')?> <small><?= $order['ORDER']['DATE_INSERT']?></small>
                                </td>
                                <td>
                                    <?
                                ...
                                <td> <?
                                    ...
                                    <?} ?></td>
                            <td>
                                <a href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_DETAIL"])?>"><span class="label label-warning"><?=Loc::getMessage('SPOL_TPL_MORE_ON_ORDER')?></span></a>
                                <a href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_COPY"])?>"><span class="label label-success"><?=Loc::getMessage('SPOL_TPL_REPEAT_ORDER') ?></span></a></td>
                            </tr>
                            <?
                        }
                        ?>
